...
#   - tailscale0: allow ALL in/out (any ports)
#   - Docker bridges: allow ALL in/out (best-effort)
#   - Ensure forwarding works (DEFAULT_FORWARD_POLICY=ACCEPT)
#   - Enable UFW now + on reboot
#
# Safety:
#   - If OPEN_WAN_443=1 and Tailscale is NOT ready => refuse (prevents lockout)
#   - Override with ALLOW_LOCKOUT=1
#
...
ALLOW_LOCKOUT="${ALLOW_LOCKOUT:-0}"                 # 1/0
...
tailscale_ready() {
  ...
  ip link show "$TAILSCALE_IFACE" >/dev/null 2>&1 || return 1
  return 0
}
...
allow_tailscale() {
  log "Allow ALL in/out on ${TAILSCALE_IFACE} (any ports)"
...
allow_docker_bridges() {
  ...
  [[ -n "$ifaces" ]] || { log "No Docker bridges found"; return 0; }
